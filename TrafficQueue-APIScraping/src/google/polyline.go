...
//overwrite and redesign for my project my tossapon
package google
...
func DecodePolyline(poly string) []geo.Point {
	p := &Polyline{
		Points: poly,
	}
	return p.Decode()
...
func (p *Polyline) Decode() []geo.Point {
	input := bytes.NewBufferString(p.Points)

	var lat, lng int64
	path := make([]geo.Point, 0, len(p.Points)/2)
	for {
...
func Encode(path []geo.Point) string {
